...
class CloudDetailViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if isExist(ruleSet.uuid) {
            subscribeButton.setTitle("Unsubscribe".localized(), for: [])
            subscribeButton.backgroundColor = "FF5E3B".color
        }else {
            subscribeButton.setTitle("Subscribe".localized(), for: [])
    ...
    func loadData() {
        ...
        API.getRuleSetDetail(ruleSet.uuid) { (response) in
            ...
            if response.result.isFailure {
                ...
                self.showTextHUD((!errDesc.isEmpty ? "\(errDesc)" : "Unkown error".localized()), dismissAfterDelay: 1.5)
            }else {
    ...
    lazy var subscribeButton: UIButton = {
        ...
        v.setTitleColor(UIColor.init(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), for: [])
        return v
    }()
